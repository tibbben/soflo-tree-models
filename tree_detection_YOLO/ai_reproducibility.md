...
&#x20; Every number in `progress\_summary.md` came from a command I ran.
...
configuration. This is the reason the results ladders in `progress\_summary.md` can be
...
a=torch.load('<run\_a>/weights/best.pt',map\_location='cpu',weights\_only=False)\['model'].state\_dict(); \\

b=torch.load('<run\_b>/weights/best.pt',map\_location='cpu',weights\_only=False)\['model'].state\_dict(); \\
...
`run\_config.yaml`, so there is a permanent record of what produced which weights.
...
\- `progress\_summary.md` — every result, including those that failed
...
limitation in `progress\_summary.md`.
...
training noise. Several conclusions in `progress\_summary.md` rest on gaps of 0.01–0.04
