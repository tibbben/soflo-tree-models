"""
chip_data.py — Tile a GeoTIFF into 640x640 chips for YOLOv8, and write matching
YOLO-format bounding-box labels from the tree points.

Run from the gables_campus/ folder. Modes:
    python scripts/chip_data.py full         # whole campus  -> images/train
...
"full" reads the whole drone survey + full labels directly (no cropped tif needed).
"training"/"validation" read the cropped ./<area>_area.tif + ./<area>_trees.geojson.
"""
...
OVERLAP = 100          # overlap between adjacent chips, in pixels
CROWN_RADIUS_M = 5.0   # assumed crown radius in meters -> half the box side
...
with rasterio.open(tiff_path) as src:
    ...
    crown_radius_px = int(CROWN_RADIUS_M / pixel_size)  # 5m / 0.05 = 100 px
    step = CHIP_SIZE - OVERLAP                          # stride

    # Slide a 640x640 window across the raster (edges included via offsets()).
    for row_off in offsets(src.height, CHIP_SIZE, step):
        for col_off in offsets(src.width, CHIP_SIZE, step):
            ...
            chip = src.read(window=window)             # (bands, H, W)
            ...
            if has_trees:
                for _, tree in chip_trees.iterrows():
                    ...
                    # Square box, clamped to chip bounds so edge trees stay valid.
            ...
            # Save the chip image (first 3 bands -> RGB) and its label file.
            name = f"chip_{chip_count:05d}"
            img = Image.fromarray(chip[:3].transpose(1, 2, 0).astype(np.uint8))
